...
public class GameController implements Runnable {
    ...
    // --- ĐƯỜNG DẪN CHUẨN ĐÓNG GÓI (Bỏ "res/") ---
    ...
    public void startNewGame() {
        ...
        currentColor = WHITE; isDraw = false; gameOver = false; isClickedToMove = false; promotion = false;
        castlingP = null; isCastling = false;
        resetTime();
        gamePanel = new GamePanel(this);
        showPanel(gamePanel);
        audioManager.playBGM(GAME_BGM);
        isTimeRunning = true; GameState.currentState = State.PLAYING;
        if (gameThread == null || !gameThread.isAlive()) { gameThread = new Thread(this); gameThread.start(); }
    }

    // --- CÁC HÀM ĐIỀU KHIỂN (Sửa lỗi image_246b90) ---
    ...
    public void resumeGame() { if (GameState.currentState == State.PAUSED) { isTimeRunning = true; GameState.setState(State.PLAYING); showPanel(gamePanel); } }
    public void exitToMenu() { isTimeRunning = false; GameState.setState(State.MENU); menuPanel.resetMenu(); audioManager.playBGM(MENU_BGM); showPanel(menuPanel); }
    ...
    private void handleMouseInput() {
        ...
        if (activeP == null) {
            for (Piece piece : simPieces) if (piece.color == currentColor && piece.col == col && piece.row == row) {
    ...
    public void simulateClickToMove(int tc, int tr) {
        ...
        if (castlingP != null) {
            ...
            castlingP.x = castlingP.getX(castlingP.col); isCastling = true;
        }
    }
    ...
    private boolean canPromote() {
        ...
        if ((activeP.color == WHITE && activeP.row == 0) || (activeP.color == BLACK && activeP.row == 7)) {
            ...
            for(Piece p : simPieces) if(p.color == currentColor) {
                if(p.type==Type.QUEEN) q++; else if(p.type==Type.ROOK) r++;
                else if(p.type==Type.BISHOP) b++; else if(p.type==Type.KNIGHT) n++;
            }
            if (q < 1) addPromo(Type.QUEEN, 7);
            if (r < 2) addPromo(Type.ROOK, 7);
            if (b < 2) addPromo(Type.BISHOP, 7);
            if (n < 2) addPromo(Type.KNIGHT, 7);
    ...
    private void addPromo(Type t, int r) {
        int c = 8 + promoPieces.size(); Piece p = null;
        ...
        if (p != null) { p.image = reloadPieceImage(p); p.x = p.getX(p.col); p.y = p.getY(p.row); promoPieces.add(p); }
    }
    ...
    private void replacePawnAndFinish(Piece p) {
        ...
        if (newP != null) { newP.image = reloadPieceImage(newP); newP.updatePosition(); simPieces.remove(activeP); simPieces.add(newP); copyPieces(simPieces, pieces); }
        audioManager.playSFX("/audio/sfx/promote.wav"); promotion = false; finalizeTurn();
    }

    private void promoting() {
        if (!mouse.released) return;
        for (Piece p : promoPieces) if (mouse.x >= p.x && mouse.x <= p.x + Board.SQUARE_SIZE && mouse.y >= p.y && mouse.y <= p.y + Board.SQUARE_SIZE) { replacePawnAndFinish(p); break; }
        mouse.released = false;
    }
    ...
    // --- CÁC HÀM GETTER ĐỂ FIX LỖI ĐỎ Ở VIEW ---
    ...
    // --- CÁC HÀM LƯU TẢI (Sửa lỗi image_23f735) ---
    ...
    public void loadGame(int s) {
        File f = new File("savegame_" + s + ".dat"); if (!f.exists()) return;
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f))) {
            SaveData d = (SaveData) ois.readObject(); pieces.clear(); pieces.addAll(d.pieces); currentColor = d.currentColor; timeLeft = d.timeLeft;
            for (Piece p : pieces) { p.image = reloadPieceImage(p); p.updatePosition(); }
            copyPieces(pieces, simPieces); gamePanel = new GamePanel(this); showPanel(gamePanel);
            audioManager.playBGM(GAME_BGM); GameState.setState(State.PLAYING); isTimeRunning = true;
            if (gameThread == null || !gameThread.isAlive()) { gameThread = new Thread(this); gameThread.start(); }
        } catch (Exception e) {}
    }
    public BufferedImage getSlotThumbnail(int s) { try { File f = new File("thumbnail_" + s + ".png"); return f.exists() ? ImageIO.read(f) : null; } catch (Exception e) { return null; } }
    public String getSlotMetadata(int s) { File f = new File("savegame_" + s + ".dat"); if (!f.exists()) return "Empty"; try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f))) { SaveData d = (SaveData) ois.readObject(); return d.saveTime; } catch (Exception e) { return "Empty"; } }

    // --- CÁC HÀM TIỆN ÍCH ---
    public boolean isInsufficientMaterial() { for (Piece p : simPieces) if (p.type == Type.PAWN || p.type == Type.ROOK || p.type == Type.QUEEN) return false; return simPieces.size() <= 3; }
    private void updateCursorState() { int col = mouse.x / Board.SQUARE_SIZE, row = mouse.y / Board.SQUARE_SIZE; boolean h = false; if (activeP == null) { for (Piece p : simPieces) if (p.color == currentColor && p.col == col && p.row == row) h = true; } else { for (int[] mv : validMoves) if (mv[0] == col && mv[1] == row) h = true; } window.setCursor(Cursor.getPredefinedCursor(h ? Cursor.HAND_CURSOR : Cursor.DEFAULT_CURSOR)); }
    public boolean isKingInCheck() { Piece king = getKing(false); if (king == null) return false; for (Piece p : simPieces) if (p.color != king.color && p.canMove(king.col, king.row)) { checkingP = p; return true; } checkingP = null; return false; }
    ...
    private void triggerEndGame(boolean d, Integer w) { this.gameOver = true; this.isTimeRunning = false; this.isDraw = d; GameState.currentState = State.GAME_OVER; if (d) audioManager.playSFX("/audio/sfx/draw.wav"); else if (w != null) audioManager.playSFX(w == WHITE ? "/audio/sfx/white_win.wav" : "/audio/sfx/black_win.wav"); }
    ...
    private void calculateValidMoves(Piece p) { validMoves.clear(); if (p == null) return; for (int r = 0; r < 8; r++) for (int c = 0; c < 8; c++) if (p.canMove(c, r) && simulateMoveAndKingSafe(p, c, r)) { int t = (p.gettingHitP(c, r) != null || (p.type == Type.PAWN && c != p.col)) ? 1 : 0; validMoves.add(new int[] { c, r, t }); } }
